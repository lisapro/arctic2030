...
map.drawmapscale(125,76.2, 125, 76.2,  200, barstyle='fancy')
'''
...
roms_x,roms_y = map(roms_lon,roms_lat)
...
poly = Polygon(((x[0],y[0]),(x[1],y[1]),(x[2],y[2]),(x[3],y[3])), facecolor='#fff7e6', alpha=1,zorder = 1,label = 'Methane seepage rate' )
ax.add_patch(poly)
plt.gca().add_patch(poly)
plt.legend(handles=[cax,cax2,poly],loc=4)
...
ax.annotate('ROMS st.\n76.74N \n126.71E ', (roms_x,roms_y), xytext=(15, -49),weight = weight, fontsize=14, #size= txtsize,
            arrowprops=dict(arrowstyle="-",connectionstyle="arc3"),
            textcoords='offset points')
...
plt.savefig('laptev_map.png', format='png',transparent=True)
