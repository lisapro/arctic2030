...
def make_nc_baseline_1995():
    ...
    seconds = [d2n(delta) for delta in np.arange(1,len(times)+1)] #np.arange(1,365*n_years+1)]    
    days = [day for day in np.arange(1,len(times)+1)] #np.arange(1,365*n_years+1)]

    nc_format = 'NETCDF3_CLASSIC'
    f1 = Dataset('Data\Laptev_baseline_1995.nc', mode='w', format= nc_format)
    f1.description="Baseline after BROM run" 
    f1.source = 'Elizaveta Protsenko (elp@niva.no)'
    f1.history = 'Created ' + time.ctime(time.time())
    ...
    def n_years(flux):    
        flux = (pd.concat([flux,flux,flux,flux,flux,flux],
               axis = 0,ignore_index=True)).iloc[:stop,:]       
        return flux
    
    flux_B1 = n_years(flux_B1)
    flux_B1_50 = n_years(flux_B1_50)
    flux_B2_10 = n_years(flux_B2_10)
    flux_B2_50 = n_years(flux_B2_50)  
    ...
    slb_year = scen.calculate_baseline(days_1)/10
    slb_year = pd.concat(
        [slb_year,slb_year,slb_year,slb_year,slb_year,slb_year],axis = 0).iloc[:stop,:]
    print(slb_year.shape)
